parameters = {'instrument': {'inst_name': 'SPHERE'}, 'pupil': {'filename': 'SPHERE/pupil=vlt_btw_nPup=800.fits', 'N': 800}, 'focal_plane_mask': {'radius': 2.252, 'num_pix': 800, 'grayscale': True, 'field_stop_radius': -1.0}, 'lyot_stop': {'filename': 'SPHERE/sphere_stop_ST_ALC2_nPup0800.fits', 'alignment_tolerance': 2, 'num_lyot_stops': 5}, 'image': {'contrast': 6, 'iwa': 5, 'owa': 20, 'num_wavelengths': 3, 'bandwidth': 0.2, 'resolution': 2}, 'method': {'force_no_x_mirror_symmetry': False, 'force_no_y_mirror_symmetry': False, 'force_no_hermitian_symmetry': False, 'starting_scale': 2, 'ending_scale': 1, 'edge_width_for_prior': 2, 'num_throughput_iterations': 2, 'initial_throughput_estimate': 1, 'maximize_planet_throughput': True}, 'solver': {'num_threads': 0, 'crossover': 0, 'method': 2}}
file_organization = {'survey_dir': '/home/mndiaye/python/aplc_optimization/surveys/sphere_refin-robust5-multilam_N0800_rita', 'solution_dir': '/home/mndiaye/python/aplc_optimization/surveys/sphere_refin-robust5-multilam_N0800_rita/solutions', 'analysis_dir': '/home/mndiaye/python/aplc_optimization/surveys/sphere_refin-robust5-multilam_N0800_rita/analysis', 'drivers_dir': '/home/mndiaye/python/aplc_optimization/surveys/sphere_refin-robust5-multilam_N0800_rita/drivers', 'log_dir': '/home/mndiaye/python/aplc_optimization/surveys/sphere_refin-robust5-multilam_N0800_rita/logs', 'input_files_dir': '/home/mndiaye/python/aplc_optimization/masks'}
solution_fname = "/home/mndiaye/python/aplc_optimization/surveys/sphere_refin-robust5-multilam_N0800_rita/solutions/0_SPHERE_N800_FPM225M0800_IWA0500_OWA02000_C6_BW20_Nlam3_LS_ID_ST_A_OD_nPu_ls_0800.fits.fits"
...
lyot_stop_lrud = np.flipud(np.fliplr(lyot_stop.shaped))


# Building Lyot stops according to alignment tolerance
if ls_num_stops in [1, 5, 9]:
    lyot_stops.extend([lyot_stop.shaped.ravel(), lyot_stop_ud.ravel(), lyot_stop_lr.ravel(), lyot_stop_lrud.ravel()])
else:
    lyot_stops = []
...
if ls_num_stops in [4, 5, 8, 9]:
    ...
    lyot_stop_pos_y = np.roll(lyot_stop.shaped, ls_alignment_tolerance, 0).ravel()
    lyot_stop_neg_y = np.roll(lyot_stop.shaped, -ls_alignment_tolerance, 0).ravel()
    lyot_stop_ud_pos_x = np.roll(np.flipud(lyot_stop.shaped), ls_alignment_tolerance, 1).ravel()
    lyot_stop_ud_neg_x = np.roll(np.flipud(lyot_stop.shaped), -ls_alignment_tolerance, 1).ravel()
    lyot_stop_ud_pos_y = np.roll(np.flipud(lyot_stop.shaped), ls_alignment_tolerance, 0).ravel()
    lyot_stop_ud_neg_y = np.roll(np.flipud(lyot_stop.shaped), -ls_alignment_tolerance, 0).ravel()
    lyot_stop_lr_pos_x = np.roll(np.fliplr(lyot_stop.shaped), ls_alignment_tolerance, 1).ravel()
    lyot_stop_lr_neg_x = np.roll(np.fliplr(lyot_stop.shaped), -ls_alignment_tolerance, 1).ravel()
    lyot_stop_lr_pos_y = np.roll(np.fliplr(lyot_stop.shaped), ls_alignment_tolerance, 0).ravel()
    lyot_stop_lr_neg_y = np.roll(np.fliplr(lyot_stop.shaped), -ls_alignment_tolerance, 0).ravel()
    lyot_stop_lrud_pos_x = np.roll(np.flipud(np.fliplr(lyot_stop.shaped)), ls_alignment_tolerance, 1).ravel()
    lyot_stop_lrud_neg_x = np.roll(np.flipud(np.fliplr(lyot_stop.shaped)), -ls_alignment_tolerance, 1).ravel()
    lyot_stop_lrud_pos_y = np.roll(np.flipud(np.fliplr(lyot_stop.shaped)), ls_alignment_tolerance, 0).ravel()
    lyot_stop_lrud_neg_y = np.roll(np.flipud(np.fliplr(lyot_stop.shaped)), -ls_alignment_tolerance, 0).ravel()

    lyot_stops.extend([lyot_stop_pos_x, lyot_stop_neg_x, lyot_stop_pos_y, lyot_stop_neg_y])
    lyot_stops.extend([lyot_stop_ud_pos_x, lyot_stop_ud_neg_x, lyot_stop_ud_pos_y, lyot_stop_ud_neg_y])
    lyot_stops.extend([lyot_stop_lr_pos_x, lyot_stop_lr_neg_x, lyot_stop_lr_pos_y, lyot_stop_lr_neg_y])
    lyot_stops.extend([lyot_stop_lrud_pos_x, lyot_stop_lrud_neg_x, lyot_stop_lrud_pos_y, lyot_stop_lrud_neg_y])
...
if ls_num_stops in [8, 9]:
    ...
    lyot_stop_ud_pos_x_pos_y = np.roll(np.roll(np.flipud(lyot_stop.shaped), ls_alignment_tolerance, 1), ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_ud_pos_x_neg_y = np.roll(np.roll(np.flipud(lyot_stop.shaped), ls_alignment_tolerance, 1), -ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_ud_neg_x_pos_y = np.roll(np.roll(np.flipud(lyot_stop.shaped), -ls_alignment_tolerance, 1), ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_ud_neg_x_neg_y = np.roll(np.roll(np.flipud(lyot_stop.shaped), -ls_alignment_tolerance, 1), -ls_alignment_tolerance,
                                    0).ravel()
    
    lyot_stop_lr_pos_x_pos_y = np.roll(np.roll(np.fliplr(lyot_stop.shaped), ls_alignment_tolerance, 1), ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_lr_pos_x_neg_y = np.roll(np.roll(np.fliplr(lyot_stop.shaped), ls_alignment_tolerance, 1), -ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_lr_neg_x_pos_y = np.roll(np.roll(np.fliplr(lyot_stop.shaped), -ls_alignment_tolerance, 1), ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_lr_neg_x_neg_y = np.roll(np.roll(np.fliplr(lyot_stop.shaped), -ls_alignment_tolerance, 1), -ls_alignment_tolerance,
                                    0).ravel()
    
    lyot_stop_lrud_pos_x_pos_y = np.roll(np.roll(np.flipud(np.fliplr(lyot_stop.shaped)), ls_alignment_tolerance, 1), ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_lrud_pos_x_neg_y = np.roll(np.roll(np.flipud(np.fliplr(lyot_stop.shaped)), ls_alignment_tolerance, 1), -ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_lrud_neg_x_pos_y = np.roll(np.roll(np.flipud(np.fliplr(lyot_stop.shaped)), -ls_alignment_tolerance, 1), ls_alignment_tolerance,
                                    0).ravel()
    lyot_stop_lrud_neg_x_neg_y = np.roll(np.roll(np.flipud(np.fliplr(lyot_stop.shaped)), -ls_alignment_tolerance, 1), -ls_alignment_tolerance,
                                    0).ravel()

    lyot_stops.extend([lyot_stop_pos_x_pos_y, lyot_stop_pos_x_neg_y, lyot_stop_neg_x_pos_y, lyot_stop_neg_x_neg_y])
    lyot_stops.extend([lyot_stop_ud_pos_x_pos_y, lyot_stop_ud_pos_x_neg_y, lyot_stop_ud_neg_x_pos_y, lyot_stop_ud_neg_x_neg_y])
    lyot_stops.extend([lyot_stop_lr_pos_x_pos_y, lyot_stop_lr_pos_x_neg_y, lyot_stop_lr_neg_x_pos_y, lyot_stop_lr_neg_x_neg_y])
    lyot_stops.extend([lyot_stop_lrud_pos_x_pos_y, lyot_stop_lrud_pos_x_neg_y, lyot_stop_lrud_neg_x_pos_y, lyot_stop_lrud_neg_x_neg_y])
